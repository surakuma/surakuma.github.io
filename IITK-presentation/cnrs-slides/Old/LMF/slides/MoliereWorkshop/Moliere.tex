...
{Alpines team, Inria Paris % Your institution for the title page
%%\medskip
%%\textit{suraj.kumar@inria.fr} % Your email address
}
\date[Moliere Workshop]{Moliere Associated Team Workshop\newline\newline \today}
...
\begin{itemize}
	...
	\item Alena Shilova  -- Inria Bordeaux, France
\end{itemize}
\end{frame}
...
{\small\vspace*{-0.25cm}
	\begin{itemize}
		\item Sequential algorithms to compute Tensor Train decomposition and approximation exist [Oseledets, 2011]  
	\end{itemize}
...
\begin{frame}{Tensor Train Decomposition [Oseledets, 2011]}
\begin{algorithm}[H]
...
\begin{frame}{Tensor Train Approximation [Oseledets, 2011]}
\begin{algorithm}[H]
...
%%	\end{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Proposed parallel algorithms based on this idea
		\end{itemize}
	\end{itemize}
}\end{frame}
...
\begin{center}
	\begin{tabular}{|l|c|}
	\hline
	$Log$ & $\log(\sum_{j=1}^{d}j i_j)$\\ \hline
	$Sin$ & $\sin(\sum_{j=1}^{d}i_j)$\\ \hline
	Inverse-Square-Root ($ISR$) & $\frac{1}{\sqrt{\sum_{j=1}^{d}i_j^2}}$\\ \hline
	Inverse-Cube-Root ($ICR$) & $\frac{1}{\sqrt[3]{\sum_{j=1}^{d}i_j^3}}$\\ \hline
	Inverse-Penta-Root ($IPR$) & $\frac{1}{\sqrt[5]{\sum_{j=1}^{d}i_j^5}}$\\ \hline
\end{tabular}
\end{center}
We consider $d=12$ and $i_j \in \{1, 2, 3, 4\}_{1\le j \le d}$. This setting produces a $12$-dimensional tensor with $4^{12}$ elements for each low rank function.
\end{frame}
...
\begin{block}{SVD vs QRCP+SVD vs RSVD for $Log$ tensor}
	...
	\hline
	Approach & Rank & compr & ne & \hthird-OA & \otta-OA\\ \hline
...
\begin{block}{Sequential performance with $Log$ tensor}
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\item LSB-SVD is very slow
			\item LSB-RSVD is much faster
		\end{itemize}
		\end{column}
		\begin{column}{0.45\textwidth}
		\begin{center}
			\includegraphics[scale=0.35]{./diagrams/single-core-performance-seq-svd-rsvd-parallel-rsvd.eps}
		\end{center}
		\end{column}
	\end{columns}
...
\end{block}
\vspace*{-0.125cm}
\begin{block}{RSVD Algorithm of LSB-RSVD}
\begin{itemize}
...
\begin{center}
	\begin{tabular}{|c|c|c|c|}
		...
		\hthird-RSVD & $\mathcal{O}(\frac{n^d}{P})$ & $\mathcal{O}(\frac{n^\frac{d}{2}}{\sqrt{P}}\log{}P)$ & $\mathcal{O}(\log{}d \log{}P)$\\ \hline
		%%		$\mathcal{O}(n\log{}n)$ 
		\otta-RSVD & $\mathcal{O}(\frac{n^d}{P})$ & $\mathcal{O}(\frac{n^{d-1}}{P}(1+\frac{\log{}P}{d}))$ & $\mathcal{O}(d\log{}P)$\\ \hline
	\end{tabular}
\end{center}
\end{frame}


%%Log tensor - with rank=6
%%Seq-svd = 2.1 sec seq-rsvd = 0.78 sec (row based randomization)seq-rsvd = 1.68 sec (col based randomization)parallel-rsvd = 0.05 sec
