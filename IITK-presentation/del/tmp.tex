...
\title[Scalable Tensor Computations]{Parallel and Scalable Tensor Tools for Modern Computing Systems} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\institute[Roma Team]{ ROMA Team\\ LIP and Inria, ENS Lyon}
...
\date[AG du LIP]{AG du LIP\\\today} % Date, can be changed to a custom date
...
\begin{block}{}
\begin{itemize}
	\item \textbf{Research goal}: Scalable and communication optimal tools for tensor computations
\end{itemize}
\end{block}
%%\begin{exampleblock}{}
%%%%\begin{itemize}
...
\title[Multi-TTM Computation]{Communication Optimal Algorithms for Multiple Tensor-Times-Matrix Computation} % The short title appears at the bottom of every slide, the full title is only on the title page
