...
\section*{Objective}
\begin{itemize}
	\item Study bounds on the minimum number of cache misses on a sequential machine for binary search and design approaches that achieve the obtained bounds
	\item Design an approach to store elements of an array that improves the cache utilization for binary search on a sequential machine
\end{itemize}
\section*{Context}
...
The element $8$ is always accessed irrespective of the $key$ element. Probability of accessing element $4$ is $probability(key\ne 8)\frac{1}{2}$. The element $12$ is accessed with the same probability. Similarly, the elements $2,6,10,14$ have same probability of getting accessed. The elements $1,3,5,7,9,11,13,15$ are accessed with minimum probability.
...
\begin{itemize}
	\item Determine the bound on the minimum number of cache misses in the worst case for binary search
	\item Design an approach and/or a data structure that achieves the lower bound for all inputs
\end{itemize}

\section*{Example}
Let us assume that the cache is empty in the beginning and each cache line can contain $3$ elements. 

There are 5 cache lines for our example. If we store elements contiguously, then there are $3$ cache misses in the worst case scenario.  
...
If the $key=15$, then $3$ cache lines will be accessed. Hence the total number of cache misses is $3$.


Now consider the following cache lines.
\[
\begin{array}{|c|c|c|}
\hline
8 & 4 & 12 \\ \hline
...
\end{array}
\]
The number of cache misses for any $key$ is at max $2$.  
