...
\begin{frame}{Extension of sequential lower bounds}
	...
	\begin{block}{Extension to paralllel machines}
		...
		\begin{lemma}
			...
			\emph{memory $M$} that performs \emph{$W$ elementary products} must
...
\end{frame}
	...
	\begin{center}
		...
		\foreach \x in {0,1,2,3}
			\foreach \y in {0,1,2,3}
			\node [scale=0.8] at (\xstep+ \x+0.5 , 3.5-\y) {p(\y,\x)};
...
%					\item Select $p_1,p_2$, and $p_3$ based on communication lower bounds
	...
	\begin{block}{Assignment 2 -- deadline Sept. 26}
		\brown{Questions:}
		\begin{itemize}
			\item Write pseudo code for 3-dimensional parallel matrix multiplication algorithm.
			\item Determine expressions for processor grid dimensions based on the lower bounds when $P$ is large and compute the data transfer costs of the algorithm with these dimensions.
		\end{itemize}
	\end{block}
...
\begin{frame}{Processor grid dimensions and data transfer costs}
	...
	\end{center}
	\begin{itemize}
		\item Data transfer costs of the algorithm match the leading terms in the lower bounds  
	\end{itemize}}
\end{frame}
