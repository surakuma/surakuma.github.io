...
\end{enumerate} 
\end{frame}
\begin{frame}{Summary of the last class}
	\begin{itemize}
		\item Course details (motivation, course structure, and assessment)
		\vfill
		\item Different ways to perform matrix multiplication
		\vfill
		\item Data transfer cost analysis for traditional matrix multiplication
		\vfill
		\item Deadline of assignment 1: Sept. 19  
	\end{itemize}
\end{frame}
...
\begin{frame}{Number of iterations with a phases of $R$ reads ($\neq M$)}
	...
	\right.
	$$
	{\small Here $N_A$, $N_B$ and $N_C$ represent the number of entries of $A$, $B$ and $C$, respectively.}
	\vfill
