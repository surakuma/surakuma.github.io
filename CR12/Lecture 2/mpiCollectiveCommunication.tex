...
\begin{itemize}
	\item {\small In each step $k (0\le k \lceil \lg p \rceil)$, process $i$ send data to process $(i-2^k) \% p$ and receives data from process $(i+2^k) \% p$}
\end{itemize}
\end{frame}
...
\begin{block}{Algorithm by Thakur et al.}
	\begin{itemize}
		\item In each step $k (1\le k\le p)$, process $i$ receives data from process $(i-k)\%p$ and send data to process $(i+k)\%p$
		\item $T=(p-1)\alpha + n\beta$
		\item $n$ is the total amount of data sent or received by any process
	\end{itemize}
\end{block}
\end{frame}
...
\vspace*{-0.25cm}{\small\begin{itemize}
	\item With an adaptation of Bruck's algorithm: $T_{bruck} = \lg p \alpha + \frac{p-1}{p} n \beta + \frac{p-1}{p} n \gamma$\vspace*{-0.1cm}
\end{itemize}}
\end{block}
\end{frame}
...
\vfill
	\begin{block}{Reduce}
		\begin{itemize}
			\item With binomial tree algorithm, $T_tree = \lceil\lg p \rceil (\alpha+n\beta+n\gamma)$
