...
\begin{columns}
	\begin{column}{0.485\linewidth}
		...
		With $b \approx M^{1/d}$, data transfer cost = $$\mathcal{O}\left(N + \frac{dNR}{M^{1-1/d}}\right)$$
	\end{column}\hfill
...
\begin{frame}{Settings to compute parallel communication lower bound}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			at the end must be at most $\frac{N + \sum_{i=1}^{d}n_iR }{P}$ words – will analyze amount of data
...
\begin{frame}{Parallel communication optimal MTTKRP algorithm}
	\small
		\begin{algorithm}[H]{\footnotesize
				...
				\begin{algorithmic}[1]
					\Require input tensor $\T{A}\in \mathbb{R}^{n_1\times \cdots \times n_d}$, factor matrices $U_j\in \mathbb{R}^{n_j\times R} \textnormal{ for } 1\le j \le d$, mode $j$, $P$ processors are arranged in $p_0\times p_1\times \cdots\times p_d$ logical processor grid
					\Ensure Updated $U_j$
...
\begin{frame}{Communication cost}
	\small
	We set $p_0 \approx \frac{(dR)^\frac{d}{2d-1}}{(N/P)^\frac{d-1}{2d-1}}$ and $p_k \approx \frac{n_k}{(Np_0/P)^\frac1d}$ for $k\in [d]$. 
...
\begin{frame}{Perspectives}
	\small
	\begin{itemize}
		\item Tight communication lower bounds for MTTKRP with small $P$ and rectangular factor matrices
		\vfill
		\item Cost analysis of several ways to perform MTTKRP
		\vfill
		\item Amount of reuse across multiple MTTKRPs
		\vfill
		\item Optimal cost of CP-ALS algorithm for an iteration (or for a set of $d$-iterations)
	\end{itemize}
\end{frame}
\end{document} 