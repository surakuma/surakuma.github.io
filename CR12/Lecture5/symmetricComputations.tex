...
\begin{block}{Assignment 3 -- deadline Oct 5}
	\brown{Question:}Obtain memory-independent communication lower bound for SYR2K. Assume that all the operations of each loop iteration are performed on the same processor, i.e, $A[i][k]*B[j][k]$ and $A[j][k]* B[i][k]$ are computed on one processor. 
\end{block}
\end{frame}
\end{document} 