...
\begin{frame}{Tensors ($n$-dimensional arrays)}
	\begin{center}
...
\begin{frame}{Tensor notations (following [Kolda and Bader, 2009])}
	\small
Let $\T{A}$ be a $d$-dimensional tensor of size $n_1\times n_2\times\cdots \times n_d$, $\T{A} \in \mathbb{R}^{n_1\times n_2\times n_d}$.
\begin{itemize}
...
\vfill
If your preferred language support $0$-based indexing then you can consider $0\le i,j,k \le 2$.
