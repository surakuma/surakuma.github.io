...
\begin{frame}{Tensor notations (following [Kolda and Bader, 2009])}
	\small
Let $\T{A}$ be a $d$-dimensional tensor of size $n_1\times n_2\times n_d$, $\T{A} \in \mathbb{R}^{n_1\times n_2\times n_d}$.
\begin{itemize}
...
\begin{frame}{Tensor preliminaries}
	...
	\end{itemize}
\vfill
		We can note that $\langle \T{A},\T{A} \rangle = ||A||^2$.
\end{frame}
...
\end{pmatrix}$$}	
\end{frame}
\begin{frame}{Tensor multiplication along $j$-mode with a matrix}
\small
...
%------------------------------------------------
\begin{frame}{Tensor Train (TT) decomposition: Product of matrices view}
