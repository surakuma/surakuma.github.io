...
\begin{frame}{ALS for computing a CP decomposition}
	\begin{algorithm}[H]{
		...
		\begin{algorithmic}
			\Require input tensor $\T{A}\in \mathbb{R}^{n_1\times n_2\times\cdots \times n_d}$, desired rank $k$, initial factor matrices $U_j\in \mathbb{R}^{n_j\times k} \textnormal{ for } 1\le j \le d$
...
\begin{itemize}
	\item $U_j$ can be chosen or randomly or by setting $k$ left singular vectors of $A_{(j)}$ for $1\le j \le d$
\end{itemize}
