...
\begin{frame}[fragile]{Analysis of traditional matrix multiplication algorithm}
	...
	//implements C=C+AB
	for i=1 to n
	 // read row i of A into fast memory  (total n^2 reads) 
	 for j=1 to n
...
\begin{enumerate}
	...
	\item {\small Compute the number of cache misses for each permutation of the loops. All matrices are stored in the row-major order in the slow memory. Size of each cache line is $L$ and the cache capacity $< <$ $\min(m,n,l)$. Assume that the cache is fully associative and the least recently used (LRU) strategy is employed to evict a cache line.}
\end{enumerate} 
\end{frame}
