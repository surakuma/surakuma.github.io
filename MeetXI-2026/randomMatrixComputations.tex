...
\author[Suraj {\sc Kumar}]{\small Hussam {\sc Al Daas}\inst{1}, Grey {\sc Ballard}\inst{2}, Laura {\sc Grigori}\inst{3}, Md Taufique {\sc Hussain}\inst{2}, \underline{Suraj {\sc Kumar}}\inst{4}, Mohammad Marufur {\sc Rahman}\inst{2} and Kathryn {\sc Rouse}\inst{5}}
...
\date[METT XI (2026)]{11th Workshop on Matrix Equations and Tensor Techniques\\\today} % Date, can be changed to a custom date
...
%	\item 
	...
	\item Amount of array accessed = $\phi_A + \phi_B + \phi_C$
\end{itemize}
\begin{center}
...
\end{center}
\begin{itemize}
	\item Communication lower bound = $\phi_A + \phi_B + \phi_C - \text{data owned by the processor} = \phi_A + \phi_B + \phi_C - \frac{n_1n_2+n_2n_3+n_1n_3}{P}$
\end{itemize}
...
\begin{frame}{Focus on $A\Omega$ and $\Omega^T A\Omega$ computations}
	\begin{block}{$B=A\Omega$ computation}
		\begin{itemize}
			\item $A \in \mathbb{R}^{n_1\times n_2}$ and $\Omega$ is a random matrix
			\vfill
	...
	\begin{block}{$B=A\Omega$ and $C=\Omega^T A\Omega$ computations together}
		\begin{itemize}
%			\item 
			\item $A$ is a $n\times n$ symmetric matrix and $\Omega$ is a random matrix
...
\begin{frame}{Approach to obtain communication lower bounds}
	\begin{minipage}{0.7\linewidth}
		...
		\begin{itemize}
			...
			\item $\phi_{i}$, $\phi_{jk}$: projections of $B$ on $i$-axis and $j-k$ plane
			\item Also implies that: $\phi_{ik}\phi_{jk} \ge Volume(B)$ and $\phi_{ij}\phi_{jk} \ge Volume(B)$
		\end{itemize}
	\end{minipage}$\quad$
...
\begin{block}{\small Constraints for load balanced computation}
	\begin{itemize}
		\item $B=A\Omega$ with $A \in \mathbb{R}^{n_1\times n_2}, B \in \mathbb{R}^{n_1\times r}$, $\Omega$ is a random matrix
	\end{itemize}
...
Let $\M{A} \in \mathbb{R}^{n\times n}$ be a symmetric positive definite matrix and $\M{\Omega} \in \mathbb{R}^{n\times r}$ be a random matrix.
	...
	\begin{block}{Constraints for load balanced computation}
		$$\M{B}=\M{A}\M{\Omega}, \quad \M{C}=\M{\Omega}^T\M{B}$$
		\begin{itemize}
			\item $\phi_A, \phi_B, \phi_C$ : projections of computations on arrays $A$, $B$, $C$
...
\begin{frame}{Parallel algorithms for $\M{B}=\M{A}\M{\Omega}$ and $\M{C}=\M{\Omega^T}\M{B}$}
	...
	\begin{algorithmic}[1]
		...
		\If{\emph{for any} $i, p_i \neq q_i$}
		\State $\M{B}_{i'k'}^{(j')} $ = \Call{Redistribute}{$\M{B}$}
		\EndIf
...
\begin{frame}{Processor grid dimensions}
	...
	\end{itemize}
\vfill
Approach 1 (\brown{\emph{redistribution}}):
\begin{itemize}
...
\end{itemize}
\vfill
Approach 2 (\brown{\emph{no-redistribution}}):
\begin{itemize}
	\item $\M{B} = \M{A}\M{\Omega}$ is the dominating computation
	\begin{itemize}
		\item Utilize the optimal processor grid of this computation to also perform $\M{C}=\M{\Omega^T}\M{B}$
	\end{itemize}
\end{itemize}
\vfill
\end{frame}
...
\begin{frame}{Conclusion and future work}
	...
	\begin{block}{Future Work}
		\begin{itemize}
			\item Explot symmetry to further reduce computation and communication costs
