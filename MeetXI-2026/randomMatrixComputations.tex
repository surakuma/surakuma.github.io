...
\date[METT XI (2026)]{11th Workshop on Matrix Equations and Tensor Techniques} % Date, can be changed to a custom date
...
%	\end{itemize}
	...
	\begin{itemize}
		\item There exists one processor that has at most $1/P$th amount of overall data -- we examine the required data transfers for this processor to establish the lower bound
...
\begin{frame}{Focus on $A\Omega$ and $\Omega^T A\Omega$ computations}
	...
	\begin{block}{$B=A\Omega$ and $C=\Omega^T A\Omega$ computations together}
		\begin{itemize}
			...
			\item Useful for Nyström approximation, $\widetilde{\M{A}} = (\M{A}\M{\Omega}) (\M{\Omega}^T\M{A}\M{\Omega})^\dag (\M{A}\M{\Omega})^T$
		\end{itemize}
	\end{block}
	\vfill
...
\begin{center}
	...
	\node[align=left,below,scale=1.2] at (7.25, -0.25) {$\phi_A + \phi_B + \phi_C =$ \\ $\qquad 2(\frac{k^2\ell m}{P})^{1/2} + \frac{\ell m}{P}$};
	\node[align=center,below,scale=1.2] at (12.75, -0.5) {$\phi_A + \phi_B + \phi_C =$ \\ $\qquad  3(\frac{k\ell m}{P})^{2/3}$};	
	\end{tikzpicture}
\end{center}
%\begin{itemize}
...
\begin{frame}{Approach to obtain communication lower bounds}
	\begin{minipage}{0.7\linewidth}
		...
		\begin{itemize}
			...
			\item $\phi_{j}$, $\phi_{ik}$: projection sizes of $B$ on $j$-axis and $i$-$k$ plane
...
%	\end{itemize}
	...
	&\quad \quad B[i][j] += A[i][k]*\Omega[k][j]
	\end{align*}
	\end{center}
	\vspace*{-0.35cm}
%	\begin{itemize}
		\item $\phi_A, \phi_B$ : projection sizes of computation on arrays $A$, $B$
...
Let $\M{A} \in \mathbb{R}^{n\times n}$ be a symmetric positive definite matrix and $\M{\Omega} \in \mathbb{R}^{n\times r}$ be a random matrix.
	...
	\begin{block}{Constraints for load balanced computation}
		...
		\begin{itemize}
			\item $\phi_A, \phi_B, \phi_C$ : projection sizes of computations on arrays $A$, $B$, $C$
...
\begin{frame}{Optimization problem and communication lower bounds}
	...
	\begin{center}
		...
		\phi_A \phi_B \geq \frac{n^2r}{P},\quad & \phi_B \phi_C \geq \frac{nr^2}{P}\\
