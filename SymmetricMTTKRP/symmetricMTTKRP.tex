...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Require $\T{X}[{T}_{p}] = \{ \T{X}[i][j][k]: i>j>k \in R_p \} \cup \{\T{X}[i][i][k]: (i,i,k)\in N_p\} \cup \{\T{X}[i][k][k]: (i,k,k)\in N_p\} \cup \{\T{X}[i][i][i]: (i,i,i)\in D_p\}$ is extended tetrahedral block owned by processor $p$.
		\Require $\M{U}\in \Real^{n\times R}$ is evenly divided into $m$ row blocks, and each row block $\M{U}[i]$ is evenly divided across a set of $q(q+1)$ processors $Q_i$ with $\M{U}[i]^{(p)}$ owned by processor $p$ so that $\M{U}[R_p]^{(p)} = \{\M{U}[i]^{(p)}: i \in R_p\}$ is a set of data blocks of $\M{U}$ owned by processor $p$. Similarly, $\M{V}[R_p]^{(p)}$ is defined and it is owned by processor  $p$.
%		\Ensure $\V{y} = \T{A}\times_2 \V{x} \times_3 \V{x}$ with $\V{y}[R_p]^{(p)}$ owned by processor $p$.
		\Ensure $\M{V}\in \Real^{n\times R}$ such that  $\M{V} = \M{X_{(1)}} (\M{U}\odot \M{U})$.
		
		
		\Function{$\M{V}[R_p]^{(p)} = \text{MTTKRP}$}{$\T{X}[{T}_{p}], \M{U}[R_p]^{(p)}, \Pi$}
		...
		\ForAll{$\T{X}[i][j][k] \in \T{X}[{T}_{p}]$} \label{line:mttkrp:startcomp}
		
%		\For{$r\in\{1,\ldots,r\}$}\Comment{Use $\T{X}[i][j][k]$ for each $r$ }
		
		\If{$i\neq j \neq k$}
		\State $\M{V}[i] = 2\M{X[i][j][k]_{(1)}} (\M{U}[k] \odot \M{U}[j]))$
		\State $\M{V}[j] = 2\M{X[i][j][k]_{(2)}} (\M{U}[k] \odot \M{U}[i]))$
		\State $\M{V}[k] = 2\M{X[i][j][k]_{(3)}} (\M{U}[j] \odot \M{U}[i]))$
...
%		\State $\M{V}[i]^{(p)} \pluseq \tilde{\M{V}}[i]^{(p)}$
		\ForAll{$p' \in Q_i \backslash \{p\}$}
		\State $\M{V}[i]^{(p)} \pluseq \bar{\M{V}}[p']$\label{line:mttkrp:reduceVi}
		\EndFor
